...
static void op_inc_p (int opcode) {
	vlog(" inc_p:%03x", act_reg->p);
	if (!(!act_reg->p && opcode == act_reg->prev_opcode)) {
		act_reg->p++;
		if (act_reg->p >= WSIZE) act_reg->p = 0;
	}//if
	//act_reg->p++;
	//if (act_reg->p >= WSIZE) act_reg->p = 0;
	/* below adopt from https://github.com/mike632t/x11-calc
	if (act_reg->p >= (WSIZE - 1)) {
		...
		if (act_reg->p > 0) {
			...
			uint16_t idx = act_reg->pc - 2;
			if (act_reg->flags&F_BANK) idx += 4096;
			int last_opcode = ((act_reg->rom_ex[idx/4] >> ((idx%4)*2)) & 0x03) | act_reg->rom[idx]<<2;
...
static void op_circulate_a_left (int opcode) {
	vlog(" circulate_a_left:%03x", opcode);
	int i, t;
	t = act_reg->a [WSIZE - 1];
	for (i = WSIZE - 1; i >= 1; i--)
...
static void op_clear_data_regs (int opcode) {
	vlog(" clear_data_regs:%03x", opcode);
	int base;
	int i, j;
...
static void op_keys_to_rom_addr (int opcode) {
	vlog(" keys_to_rom_addr:%03x", opcode);
...
static void op_display_reset_twf (int opcode) {
	vlog(" display_reset_twf:%03x", opcode);
...
static uint8_t sim_load_segments_spice(uint8_t digit) {
	...
	if (digit < 11) {			// 12th digit blank?
		...
		if (digit == 10) {
			...
			if (act_reg->b[12] & 0x04) segs = BIT7;
		}//if
		else {
			if (act_reg->b[i] == 6) {
				if (act_reg->a[i] == 9) segs |= CHAR_MINUS;
				...
				if (act_reg->b[i] == 0x01 || act_reg->b[i] == 0x05) segs |= CHAR_DOT;	// no comma
			}//else
		}//else
	}//if
	return segs;
...
static void display_scan_advance () {
	if ((--act_reg->display_scan_position) < (_is_spice ? 3 : 2)) {
...
*/
		...
		act_reg->display_scan_position = _is_spice ? 14-2 : 14-1;
	}//if
}
...
static void spice_display_scan() {
	...
	if (act_reg->flags & F_DISPLAY_ON) {
		if ((act_reg->display_scan_position == /*C_LEFT_SCAN*/13-1) && (b & 4))
		...
		if (b==1 || b==5) segs |= CHAR_DOT;		// we don't have comma segment
...
/* program in memory, key map
			(0)0f .. (9)9f, f changed to e-d-c for shifts F-G-H
			(.)4b (E)ff (-)af (+)bf (*)cf (/)df
 0100 1011 .
 0100 1010 H.
 0f 0e 0c 0d
...
static bool woodstock_execute_cycle () {
	...
	static int fault=0;
	//if (act_reg->pc == 0xa74) fault++; //act_reg->pc = 0xab1;
	if (fault == 16) return false;
	...
	//if (act_reg->pc == 0xa8e) opcode = 0;		// both 0xa8d, 0xa8e have inc_p, turn 0xa8e to nop and fix label search issue for
  // dead loop 0xa74,0xa75,0xa8d..0xa90
	//if ((act_reg->pc < 0x430 || act_reg->pc > 0x43c)
	if (1
		//&& (act_reg->pc < 0xa74 || act_reg->pc > 0xa75) && (act_reg->pc < 0xa8d || act_reg->pc > 0xa90)
		//&& (act_reg->pc < 0xae2 || act_reg->pc > 0xaeb)) 
		) {
		//vlog("+%03lx,%04lo:%04lo", act_reg->pc, act_reg->pc, opcode);
		//vlog("+%03lx:%01x%02x", act_reg->pc, opcode&0x03, act_reg->rom[idx]);
	}//if
	else {
		vlog("-");		// turn log off
	}//else
...
static void woodstock_set_rom(uint8_t which) {
	...
	switch (which) {
		case 3:		// 38c
			_is_spice = 1;
			act_reg->rom = rom_38c;
			act_reg->rom_ex = rom_38c_ex;
			break;
		case 2:		// 37e
			_is_spice = 1;
			act_reg->rom = rom_37e;
			act_reg->rom_ex = rom_37e_ex;
			break;
		default:
		case 1:		// 34c
			_is_spice = 1;
